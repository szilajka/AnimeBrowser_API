...
{
    [TestClass]
    public class AnimeInfoInactivationTests : TestBase
    {
        ...
        public void InitDb()
        {
            ...
            allAnimeInfos = new List<AnimeInfo> {
                ...
                new AnimeInfo { Id = 3, Title = "Kazama kun", Description = string.Empty, IsNsfw = false, IsActive = true },
                new AnimeInfo { Id = 4, Title = "Hanami", Description = string.Empty, IsNsfw = false, IsActive = false }
            };
        ...
        public async Task InactivateAnimeInfo_ShouldWork(long animeInfoId)
        {
            ...
            var animeInfoInactivationHandler = sp.GetService<IAnimeInfoInactivation>();
            var animeInfo = await animeInfoInactivationHandler!.Inactivate(animeInfoId);
            animeInfo.Should().NotBeNull();
            animeInfo.IsActive.Should().BeFalse();
            animeInfo.Should().BeEquivalentTo(foundAnimeInfo);
            if (foundSeasonRatings?.Any() == true)
            {
                foundSeasonRatings.Should().Match(seasonRatings => seasonRatings.All(sr => sr.IsAnimeInfoActive == false));
            }
            if (foundSeasons?.Any() == true)
            {
                foundSeasons.Should().Match(seasons => seasons.All(sr => sr.IsAnimeInfoActive == false));
            }
            if (foundEpisodeRatings?.Any() == true)
            {
                foundEpisodeRatings.Should().Match(episodeRatings => episodeRatings.All(er => er.IsAnimeInfoActive == false));
            }
            if (foundEpisodes?.Any() == true)
            {
                foundEpisodes.Should().Match(episodes => episodes.All(e => e.IsAnimeInfoActive == false));
            }
        }

        [DataTestMethod,
            DataRow(0), DataRow(-1), DataRow(-5), DataRow(-1000)]
        public async Task InactivateAnimeInfo_NotExistingId_ThrowException(long animeInfoId)
        {
            var sp = SetupDI(services =>
            {
                var animeInfoReadRepo = new Mock<IAnimeInfoRead>();
            ...
            var animeInfoInactivationHandler = sp.GetService<IAnimeInfoInactivation>();
            Func<Task> inactivateAnimeInfoFunc = async () => await animeInfoInactivationHandler!.Inactivate(animeInfoId);
            await inactivateAnimeInfoFunc.Should().ThrowAsync<NotExistingIdException>();
        }

        [DataTestMethod,
            DataRow(67), DataRow(10), DataRow(213123), DataRow(111)]
        public async Task InactivateAnimeInfo_NotExistingEpisode_ThrowException(long animeInfoId)
        {
            AnimeInfo foundAnimeInfo = null;
            var sp = SetupDI(services =>
            {
            ...
            var animeInfoInactivationHandler = sp.GetService<IAnimeInfoInactivation>();
            Func<Task> inactivateAnimeInfoFunc = async () => await animeInfoInactivationHandler!.Inactivate(animeInfoId);
            await inactivateAnimeInfoFunc.Should().ThrowAsync<NotFoundObjectException<AnimeInfo>>();
        }

        [DataTestMethod,
            DataRow(1), DataRow(2)]
        public async Task InactivateAnimeInfo_ThrowException(long animeInfoId)
        {
            var sp = SetupDI(services =>
            {
                var animeInfoReadRepo = new Mock<IAnimeInfoRead>();
            ...
            var animeInfoInactivationHandler = sp.GetService<IAnimeInfoInactivation>();
            Func<Task> inactivateAnimeInfoFunc = async () => await animeInfoInactivationHandler!.Inactivate(animeInfoId);
            await inactivateAnimeInfoFunc.Should().ThrowAsync<InvalidOperationException>();
        }
